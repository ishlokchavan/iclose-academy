...
export default async function BrowsePage() {
  ...
  const topics = await getLibraryTopics({}, user.id);

  /* Group topics by type for horizontal rows */
  const featuredTopic = topics[0] ?? null;
  const recentTopics = topics.slice(0, 12);

  /* Build rows by property type */
  const byType = new Map<string, { typeName: string; topics: typeof topics }>();
  ...
  return (
    <div className="pb-16">
      {/* ── Hero banner ─────────────────────────────────────────────────────── */}
      {featuredTopic ? (
        <div className="relative -mx-4 mb-10 h-[56vw] max-h-[520px] min-h-[260px] overflow-hidden sm:-mx-6 lg:mx-0 lg:rounded-3xl">
          {/* Thumbnail */}
          ...
          <div className="absolute inset-x-0 bottom-0 flex flex-col gap-3 p-6 sm:p-10">
            ...
            <div className="flex flex-wrap items-center gap-2 pt-1">
              ...
              <Button asChild variant="secondary" size="md" className="border-white/20 bg-white/10 text-white backdrop-blur-sm hover:bg-white/20">
      ...
      {recentTopics.length > 0 && (
        <ContentRow title="Latest">
          {recentTopics.map((t) => <TopicCard key={t.id} topic={t} />)}
        </ContentRow>
      )}
      ...
      {Array.from(byType.values()).map(({ typeName, topics: typeTopics }) => (
        <ContentRow key={typeName} title={typeName}>
          {typeTopics.map((t) => <TopicCard key={t.id} topic={t} />)}
        </ContentRow>
      ))}
