...
const DEFAULT_NEXT = "/partner/dashboard";

export function PartnerSignInForm({ next }: { next?: string }) {
  ...
  const [otpState, otpAction, otpPending] = useActionState<ActionState, FormData>(
    async (prev, formData) => {
      ...
      if (result?.success && result.nextEmail) {
        const params = new URLSearchParams({ email: result.nextEmail, next: target });
        router.push(`/partner/verify?${params.toString()}`);
      }
      return result;
    },
    null,
  ...
  return (
    <div className="space-y-6">
      <div className="space-y-1.5 text-center sm:text-left">
        ...
        <h1 className="text-[28px] font-bold tracking-tight text-ink">Sign in to your partner account</h1>
      ...
      <p className="text-[13px] text-ink-muted">
        Not a partner yet? Contact us to apply.
      </p>
    </div>
  );
}
