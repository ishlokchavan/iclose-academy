...
export function ResourceManager({
  ...
  function addLink(formData: FormData) {
    ...
    startTransition(async () => {
      const res = await createLinkResourceAction(topicId, slug, formData);
  ...
  async function handleFile(file: File) {
    ...
    const path = `${userId}/${topicId}/${Date.now()}-${safeName}.${ext}`;
    ...
    const res = await createFileResourceAction(topicId, slug, { label, storage_path: path });
    setUploading(false);
    if (res.error) {
      setError(res.error);
  ...
  function remove(id: string) {
    ...
    setError(null);
    startTransition(async () => {
      const res = await deleteResourceAction(id, slug);
      if (res.error) setError(res.error);
      else refresh();
    });
  }
    ...
    <div className="space-y-2">
      {resources.length === 0 && !adding ? (
        <p className="text-xs italic text-ink-muted">
          No resources yet. Attach links (listings, brochures) or upload files.
        </p>
      ) : null}
      ...
      {adding === "link" ? (
        ...
        >
          <Input name="label" autoFocus required placeholder="Label" />
      ...
      {adding === "file" ? (
        <div className="space-y-2 rounded-md border border-hairline bg-surface-raised p-3">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <p className="ml-auto text-[11px] text-ink-muted">Max 25 MB · any file</p>
          </div>
        </div>
      ) : null}
